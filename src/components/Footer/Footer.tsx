...
const Footer = () => {
  const navData = useFetch<NavItem[]>('nav');
  const footerData = useFetch<FooterType[]>('footer');
  const dataF0 = footerData?.[0];
  const socials = dataF0?.socials;
  const rights = dataF0?.allRights;
      ...
      <div>
        <div className="left">
          ...
          <div className="center">
            <ul>
              {navData?.map((n: NavItem, i: number) => (
                <li key={`link-${i}`}>
                  {!n.children && (
        ...
        <hr />
        <div className="right">
          <span>{rights?.date} <Link to={rights?.href ?? ''}>{rights?.company}</Link>. {rights?.text}
          </span>
          
          <div>
            {socials?.map((s: SocialItem, i: number) => (
